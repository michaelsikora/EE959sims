...
function err = sikora_platformSRPtest(indVarName,source,sampN,show_plots,saveplot)
...
mic2micLength = cos(pi/mjs_N)*mjs_radius*2; % distance between two adjacent microphones
%  White noise snr
wgnsnr = -50;
sclnos = 10^(wgnsnr/20);
...
dist2center = [0.5 2 3.5]; % 1.5 2 2.5 3 3.5 4 4.5];
...
if aa ~= 1
    for pp = 1:mjs_platnum
        mjs_platform(pp).eulOrient(mjs_pltheta(pp)+angles(aa),0); 
    end
end
...
if show_plots == 1
    ...
    if bb == 1 && saveplot == 1
        saveas(h2,[prefix, sprintf('platformallseries%d%d.png',bb,aa)]);
    end
end
...
%         axisnum = find(xyzbasis(vv,:) == 1);
    ...
    if bb == 1 && saveplot == 1
        saveas(h2,[prefix, sprintf('platform1series%d%d.png',bb,aa)]);
    end
end    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for aa = 1:sampN
    snrdbAngle(bb,aa) = errs(sampN*(bb-1)+aa,3);
%     plot3(dist2center,angles(ones(length(dist2center)).*aa)/pi*180,errs(2*(bb-1)+aa,4),'-k'); hold on;
end

figure(20);
scatter(angles.*180/pi,snrdbAngle(end,:),'ok'); hold on;
scatter(angles.*180/pi,snrdbAngle(1,:),'+r'); hold off;
xlabel('heading angle [degrees]');
ylabel('SNR dB of SRP image'); xlim([0 90]);
title({['SNR dB vs. heading angle for rotating platforms'],...
...
legend(['dist2src: ', num2str(dist2center(1))],['dist2src: ', num2str(dist2center(2))]);
