...
function err = sikora_platformSRPtest2(indVarName,source,sampN,show_plots,saveplot)
...
% !!!! not setup yet
% The following comment Headers are used in this file, use Ctrl+f
% with these to jump to that location
% !DECLARATIONS
% !DEFINE PLATFORMS
% !GENERATE SRP
% !
...
mjs_N = 3; % number of microphones per platform
...
dist2center = [0.5 1 1.5 2 2.5 3 3.5];
for bb = 1:length(dist2center)
% Microphone Platform centers
mjs_platformGroup = Platform([0 0 1.5],mjs_platnum,dist2center(bb));
...
%     mjs_pltan2src(pp,:) = [mjs_pl2src(pp,2) -mjs_pl2src(pp,1) 0];
    mjs_pltan2src(pp,:) = cross(mjs_pl2src(pp,:),[0 0 1]);
    % z axis rotations to orient endfire to source;
    q_plrot(pp,:) = [cos(mjs_pltheta(pp)/2) 0 0 sin(mjs_pltheta(pp)/2)];
    mjs_platform(pp).orient(q_plrot(pp,:));
    mjs_axis(pp,:) = mjs_pltan2src(pp,:); %quatRotateDup(q_plrot(pp,:),[1 0 0]); % REASSIGNMENT UNNECESSARY 
    mjs_axis(pp,:) = mjs_axis(pp,:)./sqrt(sum(mjs_axis(pp,:).^2));
    mjs_quaternion(pp,:) = [ mjs_cos2...
                            mjs_axis(pp,1)*mjs_sin2...
                            mjs_axis(pp,2)*mjs_sin2...
                            mjs_axis(pp,3)*mjs_sin2 ];
...
%         mjs_platform(pp).rotate(mjs_quaternion(pp,:));
...
%         normal = mjs_platform(pp).getNorm(); % Get normal to platform
%         quiver3(mjs_pcs(pp,1),mjs_pcs(pp,2),mjs_pcs(pp,3),...
%         normal(1),normal(2),normal(3));
%         currOrientation = mjs_platform(pp).getOrient('EULER'); % get
%         orientation angles
%         currOrientation.*(180/pi)
...
% im = srpframenn(sigout, gridax, mpostri, fs, prop.c, trez);
...
%%%%%%%%%%%%%%% PEAKS %%%%%%%%%%%%%%%%%%%%%%
% addpath([ toolspath,'FastPeakFind']);
% 
% % Find peaks of image and plot
% p = FastPeakFind(im);
% 
% pxx = p(1:2:end); % uninterleave x and y indexes
% pyy = p(2:2:end);
% for pp = 1:length(pyy) % get xy coordinates from indexes
%     locpxx(pp) = gridax{1}(pxx(pp));
%     locpyy(pp) = gridax{2}(pyy(pp)); 
% end
% 
if show_plots == 1 % plot red peak points on image
    figure(5);
    surf(gridax{1},gridax{2}, im); hold on;
%     plot3(locpxx,locpyy,ones(length(pxx)).*max(im(:)),'r+');
    xlabel('Xaxis [m]');
    ylabel('Yaxis [m]');
    zlabel('Zaxis [m]');
end
% 
% srppeaks = zeros(1,length(pxx)); % Get max peak
% for ll = 1:length(pxx);
%     srppeaks(ll) = im(pyy(ll),pxx(ll)); % image value at the peaks
% end
% srpmax = max(srppeaks); % get max
...
gridsize = 16; % size of region around peak
% range
...
figure(40)
    surf(regx,regy, imsourcewindow); hold on;
...
% 
% %%%%%%%%%%%%%%%%%%%%%% WORST CASE SNR %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % NOW, remove values in srppeaks where the index is in a given range
% % around the xmax and ymax
% 
...
% 
% 
% % get the peaks that are not in a box around the max peak
% noisepeaks = zeros(size(srppeaks));
% for ll = 1:length(pxx);
%     cond1 = length(find(regx ~= pxx(ll))) == length(regx);
%     cond2 = length(find(regy ~= pyy(ll))) == length(regy);
%     if cond1 % true when pxx is NOT in regionx
%         if cond2 % true when pyy is NOT in regiony        
%             noisepeaks(ll) = srppeaks(ll);
%         end
%     else
%         noisepeaks(ll) = 0;
%     end
...
% 
% % max of noise peaks
% noisemax = max(noisepeaks); % get max
% [ynmax xnmax] = find(im == noisemax); % get index of max peak
% locxnoisemax = gridax{1}(xnmax); % get coordinate of max peak
% locynoisemax = gridax{2}(ynmax);
% 
% if show_plots == 1 % plot max noise peak
%    plot3(locxnoisemax, locynoisemax, max(im(:)) ,'og', 'MarkerSize', 18,'LineWidth', 2);
% end
% 
% err_snr = srpmax/noisemax; % worst case SNR
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% figure(40)
%     surf(gridax{1},gridax{2}, imnoise); hold on;
%     err_snr = srpmax/avgnoise; % SNR with average of noise
...
if (saveplot == 1)
    saveas(h,sprintf('images/roomsim2_%d_%d.png',bb,aa));
end
...
scatter(dist2center,broadsideError,'+r'); hold off;
